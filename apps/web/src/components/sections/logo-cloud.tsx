  ...
  return (
    <section className={cn("border-y bg-muted/30", className)}>
      <div className="mx-auto max-w-6xl px-6 py-10">
        ...
        <ul
          role="list"
          aria-label={label ?? "Logos partenaires"}
          aria-labelledby={label ? "logo-cloud-label" : undefined}
          className="flex list-none flex-wrap items-center justify-center gap-8 sm:gap-12"
        >
          {logos.map((item) => (
            <li
              key={item.name}
